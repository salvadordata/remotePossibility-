...
#include <IRremote.h>         // Use IRremote instead of IRremoteESP8266
...
IRrecv irrecv(IR_RECEIVE_PIN);  // Initialize IR receiver
IRsend irsend(IR_SEND_PIN);     // Initialize IR sender
decode_results results;         // Struct to store received IR data
...
void sendIRSignal(uint32_t data, uint16_t nbits) {
  ...
  irsend.sendNEC(data, nbits);  // Send NEC formatted data (IRremote uses sendNEC directly)
